...
| LLM-грейдер отклоняет документы со специфической терминологией | Улучшить промпт грейдера: явно разрешить спец-термины, сменить критерий с «полезен» на «по той же теме» |
...
## Этап 14: Отладка LLM-грейдера (специфическая терминология)

### Проблема

При тестировании `ask_question` с вопросом про правила движения по Тилимилитрямдии инструмент вернул:

```
В документации не найдена информация по этому вопросу
```

При этом `find_relevant_docs` с тем же запросом нашёл документ с хорошими оценками (score 0.78–0.80) и вернул корректные чанки из файла `pravila-dvizheniya-edinorogov-tilimilitryamdiya.md`.

### Причина расхождения между инструментами

Два инструмента работают принципиально по-разному:

- **`find_relevant_docs`** — делает прямой поиск по векторной базе и возвращает результаты на основе косинусного сходства эмбеддингов. Высокий score означает, что векторы близки. Фильтрации через LLM нет.
- **`ask_question`** — запускает полный Corrective RAG граф: `rewrite_query → retrieve → grade_chunks → generate`. На шаге `grade_chunks` каждый найденный чанк дополнительно оценивается через LLM.

Документ успешно находился на шаге `retrieve` (тот же векторный поиск), но отклонялся на шаге `grade_chunks`.

### Корневая причина: промпт грейдера

Функция `grade_chunks` в [rag/nodes.py](rag/nodes.py) отправляла в LLM (`qwen2.5:3b`) следующий запрос:
...
Модель видела в документе незнакомые слова — «тилимилитрямдия», «рогосигнал», «радуготропа», «лунный светофор» — и расценивала их как бессмысленный текст, не имеющий отношения к реальному вопросу. Ответ: `{"relevant": false}` на все чанки.

После двух безуспешных retry (с переформулировкой запроса) граф уходил в `finalize`, `generate` получал пустой `relevant_docs` и возвращал стандартную фразу об отсутствии информации.

Проблема усугублялась тем, что критерий «содержит информацию, **полезную** для ответа» — слишком жёсткий для маленькой модели: она не может определить полезность, когда не понимает контекст предметной области.

### Схема потока при ошибке

```
retrieve → retrieved_docs: [score=0.80, score=0.79, ...] ✅ документы найдены
     ↓
grade_chunks → LLM: "тилимилитрямдия" — незнакомые слова → {"relevant": false}
     ↓
relevant_docs = [],  has_relevant_docs = False
     ↓
prepare_retry × 2 (переформулировка не помогает — проблема в грейдере, не в запросе)
     ↓
finalize → generate → "В документации не найдена информация по этому вопросу" ❌
```

### Исправление

Улучшен промпт грейдера в [rag/nodes.py:89-103](rag/nodes.py#L89-L103):
...
1. **Явное разрешение спецтерминологии** — «База знаний может содержать специализированные, технические или выдуманные термины — это нормально.» Снимает блок у маленькой модели на незнакомые слова.
2. **Смена критерия** — с «содержит полезную информацию» на «относится к той же теме». Тематическое соответствие модель может оценить даже без знания предметной области.
3. **Мягкий порог** — «хотя бы частично» — уменьшает число ложноотрицательных срабатываний.

### Урок

Corrective RAG — это дополнительный фильтр поверх векторного поиска. Если LLM-грейдер плохо откалиброван, он становится узким местом и блокирует правильные результаты. Для маленьких моделей (3B) особенно важно формулировать критерий грейдинга максимально конкретно и однозначно, не требуя от модели суждений, выходящих за пределы её возможностей.
